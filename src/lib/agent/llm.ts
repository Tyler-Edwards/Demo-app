...
export const isLlmConfigured = () =>
  Boolean(process.env.OVERMIND_API_KEY || process.env.OLLAMA_BASE_URL);
...
export const getLlmEndpoint = (): LlmEndpoint => {
  ...
  if (process.env.OVERMIND_API_KEY) {
    return {
      provider: "openai",
      url:
        process.env.OVERMIND_BASE_URL ||
        "https://api.overmindlab.ai/api/v1/chat/completions",
      model: process.env.OVERMIND_MODEL || "ft-858cb84e-qwen2-5-7b-instruct",
      headers: {
